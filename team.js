...
function addMetric(list, label, value, formatter = formatNumber, hint = "") {
  ...
  list.push({ label, value: formatter(parsed), raw: parsed, hint });
}
...
function buildTeamStatGroups(team, standing, snapshot, form) {
  ...
  addMetric(defense, "Räddningar", metricValue(metrics, ["SAVES"]));
  addMetric(defense, "xGA", metricValue(metrics, ["EXPECTED_GOALS_AGAINST", "XGA"]), (v) => formatDecimal(v, 2));

  const discipline = [];
  addMetric(discipline, "Gula kort", metricValue(metrics, ["YELLOWCARDS", "YELLOW_CARDS"]));
  addMetric(discipline, "Röda kort", metricValue(metrics, ["REDCARDS", "RED_CARDS"]));
  addMetric(discipline, "Frisparkar orsakade", metricValue(metrics, ["FOULS", "FOULS_COMMITTED"]));
  addMetric(discipline, "Frisparkar vunna", metricValue(metrics, ["FOULS_DRAWN"]));
  addMetric(discipline, "Offside", metricValue(metrics, ["OFFSIDES"]));
